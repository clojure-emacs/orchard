...
(defn clj-meta
  {:added "0.5"}
  [{:keys [dialect ns sym computed-ns unqualified-sym var-meta-whitelist]}]
  {:pre [(= dialect :clj)]}
  (let [ns (or ns computed-ns)
    ...
    (or
     ...
     ;; it's a var
     (some-> ns (m/resolve-var sym) (m/var-meta var-meta-whitelist))
...
(defn info*
  ...
  A `:var-meta-whitelist` can be optionally passed
  (defaults to `orchard.meta/var-meta-whitelist`;
   only applies to `:clj` since for `:cljs` there's no whitelisting)."
  [params]
